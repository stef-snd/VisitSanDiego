...
export class AuthService {
  ...
  public register(user: User & {password: string}): Observable<void> {
    ...
    return from(this.auth.createUserWithEmailAndPassword(user.email, user.password)).pipe(
      switchMap(() => this.userService.update(entity.email, entity)),
      tap(() => this._currentUser$.next(entity))
    )
  }

  public login(email: string, password: string): Observable<void> {
    return from(this.auth.signInWithEmailAndPassword(email, password)).pipe(
      switchMap(() => this.userService.findByKey(email)),
      map(user => this._currentUser$.next(user))
    )
  }
}
